...
//            }
            ...
            if(Math.abs(gamepad2.left_stick_y)>ignoreCutoff){
                vertSlideControl(-gamepad2.left_stick_y, true); //normal movement (overrides manual tensioning)
            }
            else{
                vertSlideControl(-gamepad2.right_stick_y, false); //manual tensioning
            }
/*
 ...
 */
    ...
    private void vertSlideControl(float power, boolean move) {
        //move true means normal movement
        //move false means tensioning (may not be needed in robot v2), but implementation carreid over from comp1
        ...
        if(move){
            if (Math.abs(power) < ignoreCutoff) {
                ...
                //For both motors at a time
                vertSlideUp.slideMotor.set(upPowerFactor * power);
                vertSlideDown.slideMotor.set(downPowerFactor * power);

                /*
                //If only need 1 motor at a time
                if (power > 0) {
                    vertSlideUp.slideMotor.set(upPowerFactor * power);
                    } else if (power < 0) {
                        vertSlideDown.slideMotor.set(downPowerFactor * power);
            ...
            //teleop manual tensioning (when move == false)
...
//        telemetry.addData("right_stick_x", gamepad1.right_stick_x);
        ...
        if (clawServo.available) {
            telemetry.addData("clawServo1", clawServo.getAngle());
        }
